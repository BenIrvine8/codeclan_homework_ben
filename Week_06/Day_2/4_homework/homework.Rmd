...
samsung_ratings %>%
  ...
  dplyr::filter(rating <= samsung_stats$mean + samsung_stats$sd) %>%
  summarise(proportion_1sd = n() / nrow(samsung_ratings))

samsung_ratings %>%
  dplyr::filter(rating >= samsung_stats$mean - 2 * samsung_stats$sd) %>%
  dplyr::filter(rating <= samsung_stats$mean + 2 * samsung_stats$sd) %>%
  summarise(proportion_2sd = n() / nrow(samsung_ratings))

samsung_ratings %>%
  dplyr::filter(rating >= samsung_stats$mean - 3 * samsung_stats$sd) %>%
  dplyr::filter(rating <= samsung_stats$mean + 3 * samsung_stats$sd) %>%
  summarise(proportion_3sd = n() / nrow(samsung_ratings))
```
...
Here's the code to filter and summarise the proportion of ratings within one 

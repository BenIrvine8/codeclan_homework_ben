...
The dimensions are 242 x 18, the variable types are double and character and the names are:
"Beverage_category", "Beverage", "Beverage_prep", "Calories",  "Total Fat (g)",  "Trans Fat (g)",  "Saturated Fat (g)",  "Sodium (mg)", "Total Carbohydrates (g)", "Cholesterol (mg)",   "Dietary Fibre (g)", "Sugars (g)", "Protein (g)", "Vitamin A (% DV)", "Vitamin C (% DV)", "Calcium (% DV)", "Iron (% DV)", "Caffeine (mg)".         

```{r}
starbucks_drinks_menu <- read_csv("data/starbucks_drinkMenu_expanded.csv")
dim(starbucks_drinks_menu)
...
```{r}
starbucks_calorie_info <- select(starbucks_drinks_menu, Beverage_category, Beverage, Beverage_prep, Calories)
is.na(starbucks_calorie_info)
```
...
```{r}
mutate(starbucks_calorie_info, calorie_diff = Calories - 135)
```

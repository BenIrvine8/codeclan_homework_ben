...
```{r}
pattern <- "[A-Z][a-z]+[\\s][0-9][a-z]{2},\\s[0-9]{4}"
  whole_text %>%
    str_extract(pattern)
```
...
```{r}
whole_text %>%
  str_extract_all("[Gg]in")



pattern <- "Gin"

whole_text_vodka <- whole_text %>%
str_replace_all(pattern, "vodka")
...
```{r}
pattern_mini <- "[Mm]ini"
pattern_m <- "[Mm]inistry"
pattern_ms <- "[Mm]inistries"
pattern_ds <- "[Dd]epartment"


whole_text_renamed <- whole_text_vodka %>%
str_replace_all(pattern_mini, "Dep") %>%
  str_replace_all(pattern_m, "Department") %>%
   str_replace_all(pattern_ms, "Departments") %>%
    str_replace_all(pattern_ds, "Sections") 
```
